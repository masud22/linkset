...
/*******************************************************************************
 * A simple single threaded implementation that is aware of an event type
 * heirarchy.
 * 
...
public final class HierarchyAwareEventBus implements EventBus {
	...
	public void fire(final Object event) throws InvocationTargetException,
		...
		for (Class<?> cls = event.getClass(); cls != Object.class; cls = cls
				.getSuperclass()) {

			final DefaultListenerManager manager = this.map.get(cls);
			if (manager != null) {
